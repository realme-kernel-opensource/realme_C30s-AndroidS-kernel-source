/***************************************************************
** Copyright (C),  2018,  tl Mobile Comm Corp.,  Ltd
** VENDOR_EDIT
** File : tl_display_private_api.h
** Description : tl display private api implement
** Version : 1.0
** Date : 2018/03/20
...
/*
 * we will create a sysfs which called /sys/kernel/tl_display,
 * In that directory, tl display private api can be called
 */
...
/*add /sys/kernel/tl_display/tp_gesture*/
...
static struct kobject *tl_display_kobj;
 ...
 * at once.
 */
static struct attribute *tl_display_attrs[] = {
	&dev_attr_cabc.attr,
	&dev_attr_tp_gesture.attr,
	NULL,	/* need to NULL terminate the list of attributes */
};

static struct attribute_group tl_display_attr_group = {
	.attrs = tl_display_attrs,
};

int tl_display_private_api_init(void)
{
	int retval;

	tl_display_kobj = kobject_create_and_add("tl_display", kernel_kobj);
	if (!tl_display_kobj)
		return -ENOMEM;

	/* Create the files associated with this kobject */
	retval = sysfs_create_group(tl_display_kobj, &tl_display_attr_group);
	if (retval)
		kobject_put(tl_display_kobj);

	return retval;
}
EXPORT_SYMBOL_GPL(tl_display_private_api_init);

void tl_display_private_api_exit(void)
{
	kobject_put(tl_display_kobj);
}
EXPORT_SYMBOL_GPL(tl_display_private_api_exit);

//module_init(tl_display_private_api_init);
//module_exit(tl_display_private_api_exit);
MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("Hujie <hujie@tl.com>");
