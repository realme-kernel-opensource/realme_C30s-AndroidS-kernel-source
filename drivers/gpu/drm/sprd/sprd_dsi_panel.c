...
#include "tl_display_private_api.h"
...
extern int tl_display_private_api_init(void);
extern void tl_display_private_api_exit(void);
...
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	if(touch_black_test != 1 && tp_gesture != 1){
			if((shutdown_flag)&&((strstr(panel->lcd_name, "icnl9911_milan"))||(strstr(panel->lcd_name, "icnl9911d_milan")))) {
				esd_tp_reset_notifier_call_chain(FTS_TP_RESET_0,NULL);
				gpiod_direction_output(panel->info.reset_gpio,0);
				mdelay(3);
		...
		} else if((shutdown_flag)&&(strstr(panel->lcd_name, "lcd_ft8006s"))) {
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	if(strstr(panel->lcd_name, "NT36672C")) {
    ...
    else if (strstr(panel->lcd_name, "lcd_ft8006s")){ // here
		if (panel->info.reset_gpio) {
			esd_tp_reset_notifier_call_chain(FTS_TP_RESET_0,NULL);
			//fts_tp_reset_pin(0);
			mdelay(7);
        ...
        if (panel->info.reset_gpio) {
			...
			//mdelay(4);
			gpiod_direction_output(panel->info.reset_gpio,1);
			mdelay(36); // LCD RST HIGH to MIPI Flash download
        }
    }
	...
	} else if(strstr(panel->lcd_name, "icnl9911_milan")) {
		if (panel->info.avdd_gpio) {
			gpiod_direction_output(panel->info.avdd_gpio, 1);
			mdelay(2);
		}
...
static int sprd_panel_disable(struct drm_panel *p)
{
	...
	if(panel->oled_bdev) {
		DRM_INFO("%s() oled_bdev \n", __func__);
		backlight_update_status(panel->oled_bdev);
	}
	...
	//7202  sleep in  for gesture (code in sleepout)
	if((touch_black_test == 1 || tp_gesture == 1)&&(strstr(panel->lcd_name, "gc7202_milan"))){
...
static int sprd_panel_enable(struct drm_panel *p)
{
	...
	if(panel->oled_bdev) {
		DRM_INFO("%s() oled_bdev  brightness = %x\n", __func__,panel->oled_bdev->props.brightness);
...
static int opl_max_normal_brightness =3768;
static void init_global_exp_backlight(void)
{
	int lut_index[41] = {0, 10, 99, 144, 187, 227, 264, 300, 334, 366, 397, 427, 456, 484, 511, 537, 563, 587, 611, 635, 658, 680,
						702, 723, 744, 764, 784, 804, 823, 842, 861, 879, 897, 915, 933, 950, 967, 984, 1000, 1016, 1023};
	int lut_value1[41] = {0, 7, 9, 14, 24, 37, 52, 69, 87, 107, 128, 150, 173, 197, 222, 248, 275, 302, 330, 358, 387, 416, 446, 
	...
	if (index_len == value_len) {
		for (i = 0; i < index_len - 1; i++) {
			index_start = lut_index[i] * opl_max_normal_brightness / 1023;
			index_end = lut_index[i+1] * opl_max_normal_brightness / 1023;
			value1_start = lut_value1[i] * opl_max_normal_brightness / 1023;
			value1_end = lut_value1[i+1] * opl_max_normal_brightness / 1023;
...
int sprd_oled_set_brightness(struct backlight_device *bdev)
{
	...
	{
		mdelay(30);
		//if(strstr(panel->lcd_name, "icnl9911_milan"))
			//mdelay(40);
	...
	bl_map = brightness;
	if ((brightness > 19) && (brightness < opl_max_normal_brightness)) {
	...
	if((shutdown_flag)&&(strstr(panel->lcd_name, "icnl9911_milan"))) {
		brightness   = 0;
	}
...
//	} else if (strncmp("lcd_ft8006s_milan_hdplus_boe", panel->lcd_name, 27) == 0) {
		} else if (strstr(panel->lcd_name, "ft8006s_milan_hdplus")){
		oled->cmds[0]->payload[1] = bl_map >> 4;
		oled->cmds[1]->payload[1] = bl_map & 0x0F;
		...
		sprd_panel_send_cmds(panel->slave,
			     ...
			     2);
		if(first_mdelay_flag) {
			sprd_panel_send_cmds(panel->slave,oled->cmds[2], 2);
		}
	} else if(strstr(panel->lcd_name, "gc7202_milan")){
		if(bl_map){
			oled->cmds[1]->payload[1] = bl_map >> 8;
			oled->cmds[1]->payload[2] = bl_map & 0xFF;
			sprd_panel_send_cmds(panel->slave,
					 oled->cmds[1],
					 3);
		} else {
			oled->cmds[0]->payload[1] = 0x28;
			sprd_panel_send_cmds(panel->slave,
					 oled->cmds[0],
					 2);
			oled->cmds[1]->payload[1] = 0;
			oled->cmds[1]->payload[2] = 0;
			sprd_panel_send_cmds(panel->slave,
					 oled->cmds[1],
					 3);
		}
	} else if ((strstr(panel->lcd_name, "icnl9911_milan"))||(strstr(panel->lcd_name, "icnl9911d_milan"))){
		if(bl_map) {
			oled->cmds[0]->payload[1] = 0x2c;
			oled->cmds[1]->payload[1] = bl_map >> 8;
			oled->cmds[1]->payload[2] = bl_map & 0xFF;

		} else {
			oled->cmds[0]->payload[1] = 0;
			oled->cmds[1]->payload[1] = 0;
			oled->cmds[1]->payload[2] = 0;
		}
		sprd_panel_send_cmds(panel->slave,
		...
		if (bl_map >= oled->cmds_total)
			bl_map = oled->cmds_total - 1;
...
static int sprd_oled_backlight_init(struct sprd_panel *panel)
{
	...
	if (!rc)
		...
		oled->bdev->props.brightness = 912;
...
static int sprd_panel_probe(struct mipi_dsi_device *slave)
{
	...
	ret = tl_display_private_api_init();
	if (ret) {
		DRM_ERROR("failed to tl_display_private_api_init\n");
	}
	/*
	 * FIXME:
