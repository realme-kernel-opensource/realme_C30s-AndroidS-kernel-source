...
extern bool charger_ic_err;
int charger_ic_err_cnt = 0;
...
static int rt9471_read(struct rt9471_charger_info *info, u8 reg, u8 *data)
{
	...
	if (ret < 0) {
		if (++charger_ic_err_cnt > 3)charger_ic_err = true;
		return ret;
	}
	charger_ic_err_cnt = 0;		
...
static int rt9471_charger_hw_init(struct rt9471_charger_info *info)
{
	...
	dev_warn(info->dev, "rt9471_charger_hw_init:have battery information is supplied\n");
		...
		// set charger ovp
		ret = rt9471_update_bits(info, RT9471_REG_VBUS, 0xc0, 0x02 << 6);
		if (ret)
			dev_err(info->dev, "set rt9471 disable WDT failed\n");
					    ...
					    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		else if (val->intval == CM_VENDOR_CHECK_CHARGER_CMD) {
			rt9471_charger_set_current(info, (2200000 - 50000));
			break;
		}
...
static void rt9471_charger_shutdown(struct i2c_client *client)
{
	struct rt9471_charger_info *info = i2c_get_clientdata(client);
	int ret = 0;

	dev_info(info->dev,"rt9471_charger_shutdown In\n");
	...
	if (info->otg_enable) {
		...
		ret = rt9471_update_bits(info, RT9471_REG_FUNCTION,
						  ...
						  0 << RT9471_OTG_EN_SHIFT);
		if (ret)
			dev_err(info->dev, "disable rt9471 otg failed ret = %d\n", ret);
    ...
    dev_info(info->dev,"rt9471_charger_shutdown End\n");
}
...
static struct i2c_driver rt9471_charger_driver = {
	...
	.shutdown = rt9471_charger_shutdown,
