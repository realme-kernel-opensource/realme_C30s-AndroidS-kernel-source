...
static int ft8006s_an_open_test(struct fts_test *tdata, bool *test_result)
{
    ...
    printk("[FTS_TS][TEST]%s: Enter min = %d, max = %d \n", __func__,min,max);
...
static int param_init_ft8006s_an(void)
{
    ...
    get_value_basic("OpenTest_DifferMax", &thr->basic.open_diff_max);

    //printk("[FTS_TS][TEST]%s: Enter delta_v = %d, min = %d, max = %d \n", __func__,thr->basic.open_delta_V \
	//	,thr->basic.open_diff_min,thr->basic.open_diff_max);
