...
#define TL_TP_LIMIT_ENABLE "tl_tp_limit_enable"
static struct proc_dir_entry *tl_tp_limit_enable;
   ...
   if(ts_data->ic_type == IC_TYPE_FT8006S_AA){
 	   ...
 	   if(ft8006saa){
		   if(black_test){
	   	   ...
	   	   FTS_ERROR("black_test ts_data->ic_type = %x,black_test = %d",ts_data->ic_type,black_test);	
		   if(black_test){
				FTS_ERROR("black_test");
				fts_test_entry(FTS_SELF_BLACK_TEST_INI,s,v);  // for ft8006s_an
				touch_black_test = 0;
		   }else{
			   	 touch_black_test = 1;
				 fts_test_entry(FTS_SELF_TEST_INI,s,v);
		   }
 		   //fts_test_entry(FTS_SELF_TEST_INI,s,v);  // for ft8006s_an
 	   }
...
static ssize_t tl_tp_limit_write(struct file *filp, const char __user *buf,
...
static ssize_t tl_tp_limit_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	char tmp[8] = {0};
	struct fts_ts_data *ts_data = fts_data;
//	struct input_dev *input_dev;
...
static const struct file_operations tl_tp_limit_fops =
{
	.write = tl_tp_limit_write,
	.read = tl_tp_limit_read,
	.owner = THIS_MODULE,
};

int FTS_tl_proc_node_init(void)
{
    ...
    tl_tp_limit_enable = proc_create(TL_TP_LIMIT_ENABLE, 0666,fts_proc_touchpanel_dir, &tl_tp_limit_fops);
	if (tl_tp_limit_enable == NULL) {
		FTS_ERROR("create proc/touchpanel/tl_tp_limit_enable Failed!\n");
		goto fail_tl_tp_limit_enable;	
		} 
     ...
     if (fts_proc_backscreen_baseline_file == NULL) {
         ...
         goto fail_tl_proc_baseline_file;
     }
