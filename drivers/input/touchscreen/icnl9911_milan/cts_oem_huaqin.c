...
struct proc_dir_entry *oem_proc_debug_infor_dir = NULL;
#define OEM_DEBUG_INFO_FOLDER				"debug_info"
struct proc_dir_entry *oem_proc_delta_file = NULL;
#define OEM_DELTA_FILENAME          		"delta"
struct proc_dir_entry *oem_proc_rawdata_file = NULL;
#define OEM_RAWDATA_FILENAME          		"rawdata"
struct proc_dir_entry *oem_proc_data_limit_file = NULL;
#define OEM_DATA_LIMIT_FILENAME         	"data_limit"
...
static int32_t black_selftest_proc_open(struct inode *inode, struct file *file)
{
    ...
    if(cts_dev->hwdata->hwid != CTS_DEV_HWID_ICNL9911D){
...
extern struct chipone_ts_data *cts_esd ;
...
/* @type true:baseline; false:diffdata */
static void cts_get_basedata(struct seq_file *s, void *v, bool type)
{
    //struct chipone_ts_data *cts_data = s->private;
    struct chipone_ts_data *cts_data = cts_esd;
    struct cts_device *cts_dev = &cts_data->cts_dev;
    s16 *rawdata = NULL;
    int ret, r, c;
    char type_str[10];
    ...
    cts_info("<I> Show %s", type_str);

    r = cts_dev->fwdata.rows;
    c = cts_dev->fwdata.cols;
    rawdata = kzalloc(r * c * 2, GFP_KERNEL);
    if (rawdata == NULL) {
        cts_info("<E> Allocate memory for %s failed", type_str);
        return;
    }
    ...
    if (ret) {
        cts_info("<E> Enable read %s failed", type_str);
        goto err_free_rawdata;
    }

    ret = cts_send_command(cts_dev, CTS_CMD_QUIT_GESTURE_MONITOR);
    if (ret) {
        cts_info("<E> Send cmd QUIT_GESTURE_MONITOR failed");
        goto err_free_rawdata;
    }
    msleep(50);
    ...
    if(ret) {
        cts_info("<E> Get %s failed", type_str);
        data_valid = false;
    }
    ret = cts_disable_get_rawdata(cts_dev);
    if (ret) {
        cts_info("<E> Disable read %s failed", type_str);
    }
...
static int cts_delta_read(struct seq_file *s, void *v)
{
    cts_get_basedata(s, v, false);
	return 0;
}
static int tp_differ_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, cts_delta_read, PDE_DATA(inode));
}
...
static int cts_baseline_read(struct seq_file *s, void *v)
{
    cts_get_basedata(s, v, true);
	return 0;
}

static int tp_rawdata_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, cts_baseline_read, PDE_DATA(inode));
}
...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
	...
	oem_proc_debug_infor_dir = proc_mkdir(OEM_DEBUG_INFO_FOLDER, CTS_proc_touchpanel_dir);
    if (oem_proc_debug_infor_dir == NULL) {
        cts_err("Create '/proc/touchpanel/"OEM_DEBUG_INFO_FOLDER"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }


	oem_proc_delta_file = proc_create_data(OEM_DELTA_FILENAME, (S_IRUGO),
									  oem_proc_debug_infor_dir, &tp_differ_proc_fops, cts_data);
	if (oem_proc_delta_file == NULL) {
		cts_err(" %s: oem_proc_delta_file create failed!\n", __func__);
		ret = -EFAULT;
		goto free_oem_data;
	}
	oem_proc_data_limit_file = proc_create_data(OEM_DATA_LIMIT_FILENAME, (S_IRUGO),
								  oem_proc_debug_infor_dir, &tp_rawdata_proc_fops, cts_data);
	if (oem_proc_data_limit_file == NULL) {
		cts_err(" %s: oem_proc_data_limit_file create failed!\n", __func__);
		ret = -EFAULT;		
		goto free_oem_data;
	}
