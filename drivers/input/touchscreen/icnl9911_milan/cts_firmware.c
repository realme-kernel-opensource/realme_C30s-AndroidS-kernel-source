...
extern char lcd_name_for_tp[50];
int get_lcd_name_from_cmdline(void)
{
	const char *lcd_name_p;

	lcd_name_p = strstr(lcd_name_for_tp, "lcd_icnl9911_milan_hdplus_boe_lxk");
        ...
        if (strcmp(firmware->name, name) == 0) {
            if (is_firmware_valid(firmware)) {
                cts_info("Found driver builtin '%s' "
					...
					if(firmware->hwid == CTS_DEV_HWID_ICNL9911D)
						return firmware;
					else if(( get_lcd_name_from_cmdline() == 1) && (i == 0))
						return firmware;
					else if(( get_lcd_name_from_cmdline() != 1) && (i != 0))
						return firmware;
    ...
    if (index < NUM_DRIVER_BUILTIN_FIRMWARE) {
        ...
        if (is_firmware_valid(firmware)) {
            cts_info("Found driver builtin '%s' "
				...
				if(firmware->hwid == CTS_DEV_HWID_ICNL9911D)
					return firmware;
				else if(( get_lcd_name_from_cmdline() == 1) && (index == 0))
					return firmware;
				else if(( get_lcd_name_from_cmdline() != 1) && (index != 0))
					return firmware;
            //return firmware;
        ...
        const struct cts_firmware *firmware, bool to_flash)
{
    ...
    if (!to_flash || !cts_dev->rtdata.has_flash) {
		...
		/* if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF0,
				0xCC33555A, 3, 1)) != 0) {
			cts_err("Write VALID flag to 0x015FF0 failed %d",
				ret);
			goto out;
		}
		if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FF8,
				firmware_info.firmware_sect_crc, 3, 1)) != 0) {
			cts_err("Write CRC to 0x015FF8 failed %d",
				ret);
			goto out;
		}
		if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x015FFC,
				firmware_info.firmware_sect_size, 3, 1)) != 0) {
			cts_err("Write LEN to 0x015FFC failed %d",
				ret);
			goto out;
		} */
        }else if(cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911D){
		if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x019FF0,
			0xCC33555A, 3, 1)) != 0) {
		cts_err("Write VALID flag to 0x015FF0 failed %d",ret);
		goto out;
		}
		if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x019FF8,
			firmware_info.firmware_sect_crc, 3, 1)) != 0) {
		cts_err("Write CRC to 0x015FF8 failed %d",ret);
		goto out;
		}
		if ((ret = cts_hw_reg_writel_retry(cts_dev, 0x019FFC,
			firmware_info.firmware_sect_size, 3, 1)) != 0) {
		cts_err("Write LEN to 0x015FFC failed %d",
			ret);
		goto out;
