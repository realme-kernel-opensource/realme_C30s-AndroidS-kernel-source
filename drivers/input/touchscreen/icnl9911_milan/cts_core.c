...
const static struct cts_sfctrl icnl9911c_sfctrl = {
    ...
    .xchg_sram_size = 1024u, /* For non firmware programming */
    .ops = &cts_sfctrlv2_ops
};
...
void cts_charge_mode_switch(int enable)
{
   ...
   if(enable)
   {
		...
		ret = cts_send_command(cts_dev, CTS_CMD_CHARGER_ATTACHED);//0x55
   }
   else
   {
		printk(" %s %d  charge out \n",__func__,__LINE__);
		ret = cts_send_command(cts_dev, CTS_CMD_CHARGER_DETACHED);//0x66
   }
	
	if(ret == 0)
		printk("switch_usb_state = %d charger mode:%d \n",enable,enable);
	else
		printk("switch_usb_state = %d charger mode:error \n",enable);	
}
