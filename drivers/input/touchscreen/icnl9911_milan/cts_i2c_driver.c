...
#include "../focaltech_touch_milan/touch.h"
                                ...
                                unsigned long action, void *data)
{
        ...
        case FTS_TP_RESET_0:        
            //cts_resume(cts_esd);
            cts_plat_set_reset(cts_esd->pdata, 0);
            break;			
...
#endif
{
	...
	#ifdef CONFIG_TOUCHSCREEN_CHIPONE_MILAN_9911D
	} else  if (strncmp("lcd_icnl9911d_milan_hdplus_dj", lcd_name_for_tp, 28) == 0) {

		get_hardware_info_data(HWID_CTP_DRIVER, "dj_v5000a_icnl9911d");
	#endif
	 } else {
		get_hardware_info_data(HWID_CTP_DRIVER, "boe_lxk_v5000a_icnl9911"); // default
	 } 
...
static int __init cts_driver_init(void)
{
    ...
    if (strncmp("lcd_icnl9911_milan_hdplus_lxk", lcd_name_for_tp, 28) == 0) {
        ...
        return i2c_add_driver(&cts_i2c_driver);
#else
        cts_info("it's lcd_icnl9911d_milan_hdplus_dj======= start cts_spi_driver");
        return spi_register_driver(&cts_spi_driver);
#endif
#endif
    } else if (strstr("lcd_icnl9911_milan_hdplus_boe_lxk", lcd_name_for_tp)) {
#ifdef CONFIG_CTS_I2C_HOST
        cts_info(" start cts_i2c_driver");
        return i2c_add_driver(&cts_i2c_driver);
#else
