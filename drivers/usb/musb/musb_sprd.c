...
#include "../../input/touchscreen/focaltech_touch_milan/touch.h"
...
static bool musb_sprd_is_connect_host(struct sprd_glue *glue)
{
	...
	if (prj_name == 2 || prj_name == 3) {
		if (type == UNKNOWN_TYPE) {
			...
			#if defined(CONFIG_SGM4154X_CHARGE_IC)
	...
	if (current_state) {
        ...
        if (glue->dr_mode == USB_DR_MODE_HOST) {
            if (!glue->vbus) {
                ...
                if (IS_ERR(glue->vbus)) {
                    dev_err(glue->dev,
                        "unable to get vbus supply, when to do regulator_disable()!\n");
                    glue->vbus = NULL;
                    goto end;
                }
				dev_info(glue->dev, "%s: To do 1 regulator_disable(glue->vbus)\n", __func__);
            ...
            {
				dev_info(glue->dev, "%s: To do 2 regulator_disable(glue->vbus)\n", __func__);
                ret = regulator_disable(glue->vbus);
                if (ret) {
                    dev_err(glue->dev,
                        "Failed to disable vbus: %d\n", ret);
...
static int musb_sprd_runtime_suspend(struct device *dev)
{
	...
	//if (glue->dr_mode == USB_DR_MODE_PERIPHERAL)       //PATCH20003181_SPRDROID12_TRUNK_22B_SPA_W22.19.1_p1_20220812-110341
	//	musb_sprd_release_all_request(musb);
