...
static sipa_dev_t *g_default_sia_for_sprd = NULL;
	...
	unsigned int cycles)
{
	...
	udelay(5000);	/* wait for owi reset, longer than 1ms */
...
void sipa_resume_for_sprd(void)
{
	if(g_default_sia_for_sprd) {
		sipa_resume(g_default_sia_for_sprd);
	}
}
EXPORT_SYMBOL(sipa_resume_for_sprd);
...
void sipa_suspend_for_sprd(void)
{
	if(g_default_sia_for_sprd) {
		sipa_suspend(g_default_sia_for_sprd);
	}
}
EXPORT_SYMBOL(sipa_suspend_for_sprd);
