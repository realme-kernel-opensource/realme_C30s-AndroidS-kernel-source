...
#define SC2721_LDO_VDDKPLED_PD		(SC2721_REGULATOR_BASE + 0x2b0)
...
static struct regulator_dev *ldo_vdd_kpled=NULL;

int kpled_enable(int val)
    {
...
EXPORT_SYMBOL(kpled_enable);

// wqw add for reset vdd_kpled end 
static int sc2721_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(regulators); i++) {
		...
		//wqw add for reset vdd_kpled sta
    if( !strcmp(regulators[i].name,"LDO_VDDKPLED") )
      {
//            printk(KERN_EMERG "-----------------------------------------------------------regulators name = %s\n",regulators[i].name);
            //create_sysfs_interfaces(rdev);
			ldo_vdd_kpled = rdev;
      }
	  // wqw add for reset vdd_kpled end
	}
