...
static unsigned long temp_brightness;
static int max_brightness_flag = 0;
static int power_on_set_brightness = 0;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (bd->ops) {
		...
		if(max_brightness_flag != 1) {
			pr_info("brightness thermal : brightness valus is %d\n", bd->props.brightness);
			temp_brightness = bd->props.brightness;
		}
		bd->props.max_brightness = maxbrightness;
		if (bd->props.brightness > maxbrightness) {
			max_brightness_flag = 1;
			temp_brightness = bd->props.brightness;
			bd->props.brightness = maxbrightness;
			backlight_update_status(bd);
		}
		if (bd->props.brightness < maxbrightness) {
			max_brightness_flag = 0;
			if(power_on_set_brightness == 1){ // First here is not update backlight
			    bd->props.brightness = temp_brightness;
			    backlight_update_status(bd);            
			    pr_info("brightness thermal : brightness1 valus is %d\n", bd->props.brightness);
			}      
		}
		power_on_set_brightness = 1;
