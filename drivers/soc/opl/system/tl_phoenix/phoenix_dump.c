...
#define FILE_DMESG "/data/persist_log/oplreserve/phoenix/kmsg"
#define PHOENIX_TEMP_DIR "/data/persist_log/oplreserve/phoenix"
#define FILE_KERNEL_EXPETION_INFO                                              \
	"/data/persist_log/oplreserve/phoenix/ERROR_KERNEL_EXCEPTION"
...
static void phx_dump_hang_opl_log(const char *);
...
static phx_action_mapping phx_log_dumper_mapping[] = {
	{ ERROR_HANG_OPL, phx_dump_hang_opl_log },
...
*/
static void phx_dump_hang_opl_log(const char *happen_time)
{
	struct task_struct *t_init;
	t_init = NULL;
	find_task_by_comm(TASK_INIT_COMM, &t_init);
	if (NULL != t_init && phx_is_filesystem_ready()) {
		PHX_KLOG_INFO("send hang opl signal %d at %s", SIGPHX_HANG,
			      happen_time);
