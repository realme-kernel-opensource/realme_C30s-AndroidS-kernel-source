...
//#include <soc/opl/system/boot_mode.h>
...
static int hang_opl_main_on = 1; /* default on */
static int hang_opl_recovery_method = RESTART_AND_RECOVERY;
...
static void phx_pet(void)
{
	...
	if(!phx_is_system_boot_completed())
    {
        ...
        {
			printk("lj: is phx_is_gsi_test, exit phe_pet\n");
            return ;
        }
    }
	...
	//if (is_userdebug() ) {
		PHX_KLOG_INFO("will not set ERROR_HANG_OPL with userdebug/aging version\n");
		return ;
	}

	PHX_KLOG_INFO("phoenix watchdog pet!\n");
	phx_set_boot_error(ERROR_HANG_OPL);
	return;
}
...
//	mtk_boot_mode = get_boot_mode();
	...
	if ((unisoc_boot_mode == NORMAL_BOOT) || (unisoc_boot_mode == ALARM_BOOT)) {    //lj ALARM_BOOT not deal
		return 1;
	} else {
		return 0;
	}
...
static int __init phx_is_normal_mode(void)
{
	...
	return phx_is_normal_mode_unisoc();
}
...
/*
	...
	if (str) {
		...
		get_option(&str, &hang_opl_main_on);
		PHX_KLOG_INFO("hang_opl_main_on %d\n", hang_opl_main_on);
	}
*/
	hang_opl_main_on = 1;
	PHX_KLOG_INFO("hang_opl_main_on %d\n", hang_opl_main_on);
...
/*	str = strstr(bootargs_ptr, "phx_rus_conf.recovery_method=");
	if (str) {
		...
		get_option(&str, &hang_opl_recovery_method);
		PHX_KLOG_INFO("hang_opl_recovery_method %d\n",
			      hang_opl_recovery_method);
	}
*/
	hang_opl_recovery_method=2;
	PHX_KLOG_INFO("hang_opl_recovery_method %d\n",hang_opl_recovery_method);
...
void __init phoenix_hlos_watchdog_init(void)
{
	...
	PHX_KLOG_INFO("phoenix hlos watchdog: %s\n",
		      hang_opl_main_on ? "on" : "off");
	if (hang_opl_main_on && phx_is_normal_mode()) {
