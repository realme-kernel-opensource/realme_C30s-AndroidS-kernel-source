...
//wqw add for sensor esd 
unsigned int SENSOR_VDD_GPIO;
static void vdd_power_enable(void)
{
gpio_direction_output(SENSOR_VDD_GPIO, 0);
gpio_set_value(SENSOR_VDD_GPIO, 0);     
}
static void vdd_power_disable(void)
{
gpio_direction_output(SENSOR_VDD_GPIO, 0);
gpio_set_value(SENSOR_VDD_GPIO, 1);     
}
//wqw add for sensor esd 
...
extern int kpled_enable(int val); //wqw add for vdd kpled reset 
static void get_dynamic_data(struct shub_data *sensor)
{
	...
	dev_err(&sensor->sensor_pdev->dev,"handle %d get dynamic data is %d\n", type, data[i]);
	
	if(type==3)
	{
		kpled_enable(0);
        vdd_power_disable();
		printk("wqw reset1 ldo_vddkpled %d\n");
        msleep(30);
		vdd_power_enable();
		kpled_enable(1);
		printk("wqw reset2 ldo_vddkpled %d\n");
	}
...
void sensor_switch_get_gpio_info(void)
{
	struct device_node *node = NULL;
	int ret = 0;
	node = of_find_compatible_node(NULL, NULL, "sprd,sensor_switch");
	if(!node){
		printk("of_find_compatible_node of\n");
		return ;
	}
	
	ret = of_get_named_gpio(node, "sensor_vdd_gpio", 0);
    printk("key=sensor_vdd_gpio, ret=%d\n", ret);
    if(ret >= 0) {
    	SENSOR_VDD_GPIO = (unsigned int )ret;
		gpio_request(SENSOR_VDD_GPIO, "SENSOR_VDD_GPIO");
    } 
