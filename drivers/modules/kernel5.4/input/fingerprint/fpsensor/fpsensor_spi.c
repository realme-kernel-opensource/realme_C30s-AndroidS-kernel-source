...
#include <../../../../../kernel/kernel5.4/include/linux/hardware_info.h>
...
static void fpsensor_gpio_free(fpsensor_data_t *fpsensor)
{
    ...
    if (fpsensor->reset_gpio != 0) {
	  ...
	  if (fpsensor->ven_gpio != 0) {
       // devm_gpio_free(dev, fpsensor->ven_gpio);
       // fpsensor->ven_gpio = 0;
    }
}
...
{
    if (fpsensor_dev->irq) {
        fpsensor_error("fpsensor_irq_free entry");
...
static int fpsensor_get_gpio_dts_info(fpsensor_data_t *fpsensor)
{
    ...
    if (ret) {
	 ...
	 // get ldo gpio resourece
    ret = fpsensor_request_named_gpio(fpsensor, "ldoven-gpios", &fpsensor->ven_gpio);
    if (ret) {
        fpsensor_error("Failed to request ven GPIO. ret = %d\n", ret);
        return -1;
    }
    // set reset direction output
	gpio_direction_output(fpsensor->ven_gpio, 1);
...
static void fpsensor_hw_power_enable(u8 onoff)
{
    //(void) onoff;
	//fpsensor_data_t *fpsensor_dev = g_fpsensor;
	fpsensor_error("fpsensor_hw_power_enable onoff = %d\n", onoff);
	if (onoff)
		gpio_direction_output(g_fpsensor->ven_gpio, 1);
	else
		gpio_direction_output(g_fpsensor->ven_gpio, 0);
}
...
static irqreturn_t fpsensor_irq(int irq, void *handle)
{
    ...
    ** since this is interrupt context (other thread...) */
	...
	__pm_wakeup_event(gProcessWakeLock, msecs_to_jiffies(1000));
#else	
...
{
    fpsensor_debug("fpsensor_dev_cleanup entry\n");
    if (fpsensor->cleanup_dev)
    {
        fpsensor_debug("fpsensor already cleanup\n");
        return;
    }
#if FPSENSOR_INPUT
    ...
    fpsensor->cleanup_dev = 1;
    fpsensor_debug("fpsensor_dev_cleanup exit\n");
}
...
static long fpsensor_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case FPSENSOR_IOC_REMOVE:
	  ...
	  #endif
#endif
		fpsensor_info("FPSENSOR_IOC_REMOVE\n");
        fpsensor_hw_power_enable(0);
...
static int fpsensor_release(struct inode *inode, struct file *filp)
{
    ...
    fpsensor_error("fpsensor_release entry\n");
    ...
    fpsensor_dev->device_available = 0;
    fpsensor_debug("fpsensor_release exit\n");
    return status;
}
...
#endif
gProcessWakeLock = wakeup_source_register(NULL, "fpsensor_ttw_wl");
if (!gProcessWakeLock) {
		return -ENOMEM;
	}
    fpsensor_dev->device_available = 1;
...
#endif
#endif
	get_hardware_info_data(HWID_FINGERPRINT,"ICNF7319");	
...
{
    fpsensor_data_t *fpsensor_dev = g_fpsensor;
    fpsensor_error("fpsensor_remove entry\n");
    if (fpsensor_dev != NULL) {
        kfree(fpsensor_dev);
        fpsensor_dev = NULL;
    }
